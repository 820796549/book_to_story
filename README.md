...
仓库根目录只放系统模板、文档、脚本和样板。`.active-book` 可写入当前书籍项目的相对路径，例如：

```text
projects/毛选第一卷
```

这样多人使用或同时拆多本书时，系统和内容不会混在一起。

## 本地工作区

以下路径默认被 `.gitignore` 排除，适合放用户自己的生产材料：

```
projects/*
```

根目录下的 `00-项目定义/` 到 `08-项目打包/` 也仍被忽略，只作为旧项目兼容；新项目优先使用 `projects/<书名>/`。
