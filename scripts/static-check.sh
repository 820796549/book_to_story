...
if [ -d "$REPO_ROOT/skills" ]; then
  ...
  echo "Error: skills/ or .agents/skills/ not found"
  exit 1
fi
...
check_skill() {
  ...
  local agent_names=()
  if [ -d "$SKILLS_DIR/story-setup/references/templates/agents" ]; then
    for f in "$SKILLS_DIR/story-setup/references/templates/agents/"*.md; do
